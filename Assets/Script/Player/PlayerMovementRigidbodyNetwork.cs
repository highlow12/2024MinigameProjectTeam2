...
public class PlayerMovementRigidbodyNetwork : NetworkBehaviour
{
    ...
    [SerializeField] float _dashDuration = 0.5f; // �뽬 ���� �ð�
    [SerializeField] float _dashDistance = 3.0f; // �뽬 �Ÿ�
    ...
    private void Roll(bool dash)
    {
        if (dash || CalculateRollBuffer())
        {
            ...
            if (IsGrounded && (dash) && !_isDashing) // IsGrounded�� �÷��̾ ���� �ִ��� Ȯ���ϴ� �޼����� ����
            {
    ...
    private IEnumerator DashCoroutine()
    {
        ...
        Vector2 dashDirection = _rb.Rigidbody.velocity.normalized; // ������Ʈ�� ���� �ִ� ���� (������)
        _rb.Rigidbody.velocity = dashDirection * Vector2.right * _dashDistance / _dashDuration;

        yield return new WaitForSeconds(_dashDuration);

        _rb.Rigidbody.velocity = Vector2.zero; // �뽬 �� �ӵ� �ʱ�ȭ
        _isDashing = false;
    }
}
