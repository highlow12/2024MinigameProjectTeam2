...
public class NetworkPlayerMovement : NetworkBehaviour
{
    ...
    private void OnValidate()
    {
        ...
        // Rigidbody2D의 중력 스케일을 계산합니다(즉, unity의 중력 값에 대한 중력 강도입니다, project settings/Physics2D 참조).
    ...
    #region COMPONENTS
    public NetworkRigidbody2D RB { get; private set; }
	...
	public override void FixedUpdateNetwork()
	{
		...
		#region JUMP CHECKS
		if (IsJumping && RB.Rigidbody.velocity.y < 0)
		{
			IsJumping = false;
		...
		if (!_isDashAttacking)
		{
			...
			else if (RB.Rigidbody.velocity.y < 0 && _moveInput.y < 0)
			{
				//Much higher gravity if holding down
				SetGravityScale( gravityScale *  fastFallGravityMult);
                //Caps maximum fall speed, so when falling over large distances we don't accelerate to insanely high speeds
                RB.Rigidbody.velocity = new Vector2(RB.Rigidbody.velocity.x, Mathf.Max(RB.Rigidbody.velocity.y, - maxFastFallSpeed));
			}
			else if (_isJumpCut)
			{
				//Higher gravity if jump button released
				SetGravityScale( gravityScale *  jumpCutGravityMult);
				RB.Rigidbody.velocity = new Vector2(RB.Rigidbody.velocity.x, Mathf.Max(RB.Rigidbody.velocity.y, - maxFallSpeed));
			}
			else if ((IsJumping || IsWallJumping || _isJumpFalling) && Mathf.Abs(RB.Rigidbody.velocity.y) <  jumpHangTimeThreshold)
			{
				SetGravityScale( gravityScale *  jumpHangGravityMult);
			}
			else if (RB.Rigidbody.velocity.y < 0)
			{
	...
	{
		RB.Rigidbody.gravityScale = scale;
	}
    ...
    private void Run(float lerpAmount)
	{
		...
		targetSpeed = Mathf.Lerp(RB.Rigidbody.velocity.x, targetSpeed, lerpAmount);
		...
		if ((IsJumping || IsWallJumping || _isJumpFalling) && Mathf.Abs(RB.Rigidbody.velocity.y) <  jumpHangTimeThreshold)
		{
		...
		if( doConserveMomentum && Mathf.Abs(RB.Rigidbody.velocity.x) > Mathf.Abs(targetSpeed) && Mathf.Sign(RB.Rigidbody.velocity.x) == Mathf.Sign(targetSpeed) && Mathf.Abs(targetSpeed) > 0.01f && LastOnGroundTime < 0)
		{
		...
		float speedDif = targetSpeed - RB.Rigidbody.velocity.x;
		//Calculate force along x-axis to apply to thr player

		float movement = speedDif * accelRate;

        //Convert this to a vector and apply to Rigidbody2D
        RB.Rigidbody.AddForce(movement * Vector2.right, ForceMode2D.Force);

		/*
		 * For those interested here is what AddForce() will do
		 * RB.RigidbodyRigidbody.velocity = new Vector2(RB.Rigidbody.velocity.x + (Time.fixedDeltaTime  * speedDif * accelRate) / RB.mass, RB.Rigidbody.velocity.y);
    ...
    private void Jump()
	{
		...
		if (RB.Rigidbody.velocity.y < 0)
			force -= RB.Rigidbody.velocity.y;

		RB.Rigidbody.AddForce(Vector2.up * force, ForceMode2D.Impulse);
		#endregion
	}
	...
	private void WallJump(int dir)
	{
		...
		if (Mathf.Sign(RB.Rigidbody.velocity.x) != Mathf.Sign(force.x))
			force.x -= RB.Rigidbody.velocity.x;

		if (RB.Rigidbody.velocity.y < 0) //checks whether player is falling, if so we subtract the velocity.y (counteracting force of gravity). This ensures the player always reaches our desired jump force or greater
			force.y -= RB.Rigidbody.velocity.y;

		//Unlike in the run we want to use the Impulse mode.
		//The default mode will apply are force instantly ignoring masss
		RB.Rigidbody.AddForce(force, ForceMode2D.Impulse);
		#endregion
	}
	#endregion
	...
	private IEnumerator StartDash(Vector2 dir)
	{
		...
		{
			RB.Rigidbody.velocity = dir.normalized *  dashSpeed;
		...
		SetGravityScale( gravityScale);
		RB.Rigidbody.velocity =  dashEndSpeed * dir.normalized;
	...
	private void Slide()
	{
		...
		if(RB.Rigidbody.velocity.y > 0)
		{
		    RB.Rigidbody.AddForce(-RB.Rigidbody.velocity.y * Vector2.up,ForceMode2D.Impulse);
		}
	
		//Works the same as the Run but only in the y-axis
		//THis seems to work fine, buit maybe you'll find a better way to implement a slide into this system
		float speedDif =  slideSpeed - RB.Rigidbody.velocity.y;	
		float movement = speedDif *  slideAccel;
		//So, we clamp the movement here to prevent any over corrections (these aren't noticeable in the Run)
		//The force applied can't be greater than the (negative) speedDifference * by how many times a second FixedUpdate() is called. For more info research how force are applied to rigidbodies.
		movement = Mathf.Clamp(movement, -Mathf.Abs(speedDif)  * (1 / Time.fixedDeltaTime), Mathf.Abs(speedDif) * (1 / Time.fixedDeltaTime));

		RB.Rigidbody.AddForce(movement * Vector2.up);
	}
    #endregion
    ...
    {
		return IsJumping && RB.Rigidbody.velocity.y > 0;
    }

	private bool CanWallJumpCut()
	{
		return IsWallJumping && RB.Rigidbody.velocity.y > 0;
	}
