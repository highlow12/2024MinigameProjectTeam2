...
{//Scriptable object which holds all the player's movement parameters. If you don't want to use it
 //just paste in all the parameters, though you will need to manuly change all references in this script
    ...
    [Networked] private NetworkBool _isJumpFalling { get; set; }

    //Wall Jump
    [Networked] private float _wallJumpStartTime { get; set; }
    [Networked] private int _lastWallJumpDir { get; set; }

    //Dash
    [Networked] private int _dashesLeft { get; set; }
    [Networked] private NetworkBool _dashRefilling { get; set; }
    [Networked] private Vector2 _lastDashDir { get; set; }
    [Networked] private NetworkBool _isDashAttacking { get; set; }
    ...
    public void CheckDirectionToFace(NetworkBool isMovingRight)
    {
        if (isMovingRight != IsFacingRight)
            Turn();
    }

    private NetworkBool CanJump()
    {
        return LastOnGroundTime > 0 && !IsJumping;
    }

    private NetworkBool CanWallJump()
    {
        return LastPressedJumpTime > 0 && LastOnWallTime > 0 && LastOnGroundTime <= 0 && (!IsWallJumping ||
             (LastOnWallRightTime > 0 && _lastWallJumpDir == 1) || (LastOnWallLeftTime > 0 && _lastWallJumpDir == -1));
    }

    private NetworkBool CanJumpCut()
    {
        return IsJumping && RB.velocity.y > 0;
    }

    private NetworkBool CanWallJumpCut()
    {
        return IsWallJumping && RB.velocity.y > 0;
    }

    private NetworkBool CanDash()
    {
